...
@route('/autotag', method='GET')
def autotag():
    #处理未打标
    rate_value = None
    rate_type = None
    items, page_info = get_items(
        rate_type=rate_type, rate_value=rate_value, page=1)
    for a in items:
        print(a.fanhao, json.loads(a.meta_info)['cover_img_url'])
        # 拿到番号后下载封面
        cover_url = json.loads(a.meta_info)['cover_img_url']
        # 图片格式必须为png
        proxy = APP_CONFIG['download.proxy']
        personal_url = APP_CONFIG['download.personal_url']
        proxies = {
            'http': proxy,  # 本地的代理转发
            'https': proxy
        }
        ...
        if proxy:
            r = session.get(cover_url, proxies=proxies, timeout=10)
        else:
            r = session.get(cover_url, timeout=10)
    ...
    items, page_info = get_items(
        rate_type=2, page=1)
    #type=2为系统预测value=0为预测不喜欢，value=1为预测喜欢
    for a in items:
        print(a.fanhao, json.loads(a.meta_info)['cover_img_url'])
        #拿到番号后下载封面
        cover_url = json.loads(a.meta_info)['cover_img_url']
        ...
        r = requests.get(cover_url, proxies=proxies, timeout=10)
        ...
        if face_score > 9:
            print('番号' + a.fanhao + "标记为喜欢")
            if a.rate_value == 0:
                core.nope(a.fanhao, str(a.rate_value))
            else:
                core.yes(a.fanhao, str(a.rate_value))
            print('番号' + a.fanhao + "标记为喜欢")
        else:
            if a.rate_value == 0:
                core.yes(a.fanhao, str(a.rate_value))
...
def start_app():
    ...
    # run(host='0.0.0.0', port=8880, debug=True, reloader=False)
